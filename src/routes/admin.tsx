...
function AdminPage() {
  ...
  // stats — only real (human) clicks count toward revenue
  ...
  async function loadAll() {
    const [usersRes, ledgerRes, profilesRes, withdrawRes, msgRes] = await Promise.all([
      supabase.from("profiles").select("id", { count: "exact", head: true }),
      supabase
        .from("earnings_ledger")
        .select("total_clicks, adsterra_clicks, user_clicks, earnings_usd"),
      supabase.from("profiles").select("balance_withdrawn"),
      supabase
        .from("withdrawals")
        .select("id, user_id, amount_usd, network, wallet_address, status, created_at")
    ...
    let real = 0;
    let ads = 0;
    for (const r of (ledgerRes.data as any[] | null) ?? []) {
    ...
    const withdrawn = ((profilesRes.data as any[] | null) ?? []).reduce(
  ...
  async function updateWithdrawal(id: string, status: "approved" | "rejected") {
        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <Stat
              icon={Megaphone}
              label="Adsterra (4%)"
              value={adsterraClicks.toLocaleString()}
            />
            <Stat
              icon={DollarSign}
              label="User revenue"
              value={`$${revenueUsd.toFixed(4)}`}
            />
            <Stat
              icon={Wallet}
              label="Paid out"
              value={`$${paidOut.toFixed(2)}`}
              sub={`Pending $${pendingPayouts.toFixed(2)}`}
            />
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-border bg-card p-6 md:p-8">
          ...
          <div className="flex gap-2 mb-4">
            <Button
              ...
              className={mode === "broadcast" ? "bg-primary-gradient" : ""}
            >
            ...
            <Button
              ...
              className={mode === "single" ? "bg-primary-gradient" : ""}
            >
          ...
          {withdrawals.length === 0 ? (
            ...
            <div className="space-y-2">
              {withdrawals.map((w) => (
                <div
                  key={w.id}
                  className="rounded-xl border border-border bg-background/40 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3"
                >
                  <div className="min-w-0">
                    <div className="font-display font-bold text-gradient text-lg">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {w.status === "pending" && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => updateWithdrawal(w.id, "approved")}>
                          <Check className="h-3.5 w-3.5 mr-1" /> Approve
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => updateWithdrawal(w.id, "rejected")}>
          ...
          {messages.length === 0 ? (
            ...
            <div className="space-y-2">
              {messages.map((m) => (
                <div
                  key={m.id}
                  className="rounded-xl border border-border bg-background/40 p-4 flex items-start justify-between gap-3"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-display font-semibold">{m.subject}</span>
                      {m.is_broadcast ? (
                        <span className="rounded-full bg-primary/15 text-primary text-[10px] px-2 py-0.5">
                          broadcast
                        </span>
                      ) : (
