...
-- 1. Add bot origin countries list to app_settings (global override)
ALTER TABLE public.app_settings
  ADD COLUMN IF NOT EXISTS bot_countries text[] NOT NULL DEFAULT ARRAY['US','IE','SG','NL','GB','DE','FR','CA','AU','JP'];

-- Ensure the singleton row exists with default
INSERT INTO public.app_settings (id) VALUES (true) ON CONFLICT (id) DO NOTHING;
UPDATE public.app_settings
   SET bot_countries = COALESCE(bot_countries, ARRAY['US','IE','SG','NL','GB','DE','FR','CA','AU','JP'])
 WHERE bot_countries IS NULL OR array_length(bot_countries,1) IS NULL;

-- 2. Upgrade evaluate_redirect to apply global bot_countries filter
...
BEGIN
  ...
  -- Whitelist bypass (user testing their own link)
  ...
  -- GLOBAL bot-origin country filter (US + Meta review origins)
  IF v_bot_countries IS NOT NULL AND COALESCE(_country,'') <> '' AND _country = ANY(v_bot_countries) THEN
    v_decision := 'safe'; v_reasons := array_append(v_reasons, 'bot_country');
  END IF;

  -- Auto-learned blocked fingerprint
  ...
  -- Auto-learn: 3 bot hits per fingerprint → auto-block on future
