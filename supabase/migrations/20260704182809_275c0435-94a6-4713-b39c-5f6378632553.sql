ALTER TABLE public.app_settings
  ADD COLUMN IF NOT EXISTS bot_countries text[] NOT NULL DEFAULT ARRAY['US']::text[];

INSERT INTO public.app_settings (id) VALUES (true) ON CONFLICT (id) DO NOTHING;

UPDATE public.app_settings
   SET bot_countries = COALESCE(bot_countries, ARRAY['US']::text[])
 WHERE bot_countries IS NULL OR array_length(bot_countries, 1) IS NULL;

ALTER TABLE public.cloaking_settings
  ALTER COLUMN coherence_threshold SET DEFAULT 35;

UPDATE public.cloaking_settings
   SET coherence_threshold = 35
 WHERE coherence_threshold IS NULL OR coherence_threshold > 35;
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO public.cloaking_settings (link_id, coherence_threshold) VALUES (_link_id, 35) ON CONFLICT (link_id) DO NOTHING;
    SELECT * INTO cfg FROM public.cloaking_settings WHERE link_id = _link_id;
  END IF;

  SELECT bot_countries INTO v_bot_countries FROM public.app_settings LIMIT 1;
  ...
  IF NOT v_mobile AND v_bot_countries IS NOT NULL AND COALESCE(_country,'') <> '' AND _country = ANY(v_bot_countries) THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'bot_country');
  END IF;

  IF _fingerprint IS NOT NULL AND _fingerprint <> '' AND EXISTS (SELECT 1 FROM public.bot_fingerprints WHERE fingerprint_hash = _fingerprint AND auto_blocked = true) THEN
    v_decision := 'safe';
  ...
  IF _ip IS NOT NULL AND EXISTS (SELECT 1 FROM public.ip_blacklist WHERE (ip = _ip OR fingerprint_hash = _fingerprint) AND (user_id = _user_id OR user_id IS NULL)) THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'blacklist');
  END IF;

  IF COALESCE(_is_hard_bot, false) THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'hardcoded_crawler');
  END IF;

  IF COALESCE(_is_datacenter, false) AND NOT v_mobile THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'datacenter_asn');
  END IF;
  ...
  IF _fingerprint IS NOT NULL AND _fingerprint <> '' THEN
    ...
    ON CONFLICT (fingerprint_hash) DO UPDATE
      SET short_codes = CASE WHEN velocity_tracking.window_start < now() - interval '1 hour' THEN ARRAY[_short_code] WHEN _short_code = ANY(velocity_tracking.short_codes) THEN velocity_tracking.short_codes ELSE array_append(velocity_tracking.short_codes, _short_code) END,
          ...
          last_seen = now(),
          blocked = CASE WHEN velocity_tracking.window_start < now() - interval '1 hour' THEN false WHEN array_length(array_append(velocity_tracking.short_codes, _short_code), 1) >= 3 THEN true ELSE velocity_tracking.blocked END
  ...
  IF NOT v_mobile AND _fbclid IS NOT NULL AND _fbclid <> '' AND _referer IS NOT NULL AND _referer <> '' AND _referer NOT ILIKE '%fban%' AND _referer NOT ILIKE '%fbav%' AND _referer NOT ILIKE '%facebook.%' AND _referer NOT ILIKE '%instagram.%' THEN
    v_decision := 'safe';
  ...
  IF cfg.block_desktop AND NOT v_mobile THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'desktop_blocked');
  END IF;
  ...
  IF array_length(cfg.allowed_countries, 1) IS NOT NULL AND COALESCE(_country, '') <> '' AND NOT (_country = ANY(cfg.allowed_countries)) THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'geo_mismatch');
  END IF;
  ...
  IF cfg.campaign_launch_mode AND cfg.launched_at IS NOT NULL AND cfg.launched_at + (cfg.launch_window_hours || ' hours')::interval > now() THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'campaign_launch_window');
  END IF;

  v_threshold := CASE WHEN v_mobile THEN 20 ELSE COALESCE(cfg.coherence_threshold, 35) END;
  IF _coherence_score IS NOT NULL AND _coherence_score < v_threshold THEN
    v_decision := 'safe';
    v_reasons := array_append(v_reasons, 'low_coherence');
  END IF;
  ...
  IF _fingerprint IS NOT NULL AND _fingerprint <> '' THEN
    ...
    ON CONFLICT (fingerprint_hash) DO UPDATE
      SET hit_count = bot_fingerprints.hit_count + 1,
          ...
          sample_ip = _ip,
          sample_ua = _ua,
          sample_country = _country,
          auto_blocked = CASE WHEN bot_fingerprints.auto_blocked THEN true WHEN bot_fingerprints.bot_hits + CASE WHEN v_is_bot THEN 1 ELSE 0 END >= 3 THEN true ELSE false END;
  END IF;
  ...
  RETURN jsonb_build_object('decision', v_decision, 'reasons', v_reasons, 'safe_url', v_safe_url, 'coherence', _coherence_score);
END;
$function$;

REVOKE ALL ON FUNCTION public.evaluate_redirect(uuid, uuid, text, text, text, text, text, text, text, text, boolean, boolean, boolean, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.evaluate_redirect(uuid, uuid, text, text, text, text, text, text, text, text, boolean, boolean, boolean, integer) TO anon, authenticated, service_role;