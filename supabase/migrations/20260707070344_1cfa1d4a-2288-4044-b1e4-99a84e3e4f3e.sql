...
ALTER TABLE public.app_settings
  ADD COLUMN IF NOT EXISTS monitor_mode boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS monitor_mode_until timestamptz;

UPDATE public.app_settings
   SET monitor_mode = true,
       monitor_mode_until = now() + interval '7 days';
...
BEGIN
  ...
  IF _fingerprint IS NOT NULL AND _fingerprint <> ''
     AND EXISTS (SELECT 1 FROM public.bot_fingerprints WHERE fingerprint_hash = _fingerprint AND auto_blocked = true) THEN
  ...
  IF _fingerprint IS NOT NULL AND _fingerprint <> '' THEN
    ...
    ON CONFLICT (fingerprint_hash) DO UPDATE
      SET hit_count = bot_fingerprints.hit_count + 1,
          ...
          auto_blocked = CASE WHEN bot_fingerprints.auto_blocked THEN true
